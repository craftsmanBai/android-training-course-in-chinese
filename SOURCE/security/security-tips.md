> 编写:[Zing](https://github.com/craftsmanBai),参考
原文:
# 安全要点
...
###使用内存储器
默认情况下，你在[内存储器](http://)中创建的文件只有你的app可以访问。这种机制被Android加强了并且对于大多数应用程序都是有效的。
你应该避免在IPC文件中使用[MODE_WORLD_WRITEABLE](http://)或者[MODE_WORLD_READABLE](http://)模式，因为它们不对特殊程序提供限制数据访问的功能，它们也不对数据格式提供任何控制。如果你想同其他app的进程共享数据，你可以使用一个[content provider](http://)，它给其他apps提供了可读可写的权限并且可以逐项动态的获取权限。

如果想对一些敏感数据提供特别的保护，你可以选择使用应用程序无法直接获取的密钥来加密本地文件。例如，密钥可以存放在[密钥库](http://)，使用用户密码保护的密钥，而不存储在设备上。尽管这种方式不能在具有root权限时监视用户输入的密码的情况下保护数据，但是它可以提供对没有进行[文件系统加密](http://)的丢失的设备的保护。
